	...
	.setup({
	  onReady : function() {
					...
					if (today.getHours() > 2) {
				 	...
				 	var selectedDate = maxValue.format('Y-m-d');
				  ...
				  calendarView = new Ext.ux.TouchCalendarView({
					minDate : new Date("2012-01-04"),
							   ...
							   onSelectionChange : function(selectionModel, records) {
								 if (records.length > 0) {
								   var destDate = records[0].get('date');
								   if (destDate > maxValue) {
								   	 destDate = maxValue;
								   	 this.selectDate(destDate);
								   }
								   selectedDate = destDate.format('Y-m-d');
				  ...
				  inner.show();
				  updateMondai(selectedDate);
				  self.checkNewMondai = function() {
				  	background.checkNewMondai();
				  }
				  setInterval("self.checkNewMondai()",1000);
				}
	});
